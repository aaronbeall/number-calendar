 ...
 * This is tailored for the goals generated by goal-builder and is used for grouping goals in the UI.
 */
export function getGoalCategory(goal: Goal) {
  const { type, target, count, timePeriod, badge } = goal;
  ...
  if (timePeriod === 'anytime') {
    if (target.metric === 'count' && count > 1) return `All-time Entries`;
    if (count == 1) return "Firsts";
    return 'One-time';
  }
  const period = capitalize(timePeriod);
  const periodly = ({
    day: 'Daily',
    week: 'Weekly',
    month: 'Monthly',
  ...
  if ((target.metric === 'min' || target.metric === 'max') && target.value === 0) return `Perfect`;
  if ((target.metric === 'mean' || target.metric === 'median') && target.value === 0) return `Sustain`;
  if (target.value === 0) return `${periodly} Trend` as const;
  if (badge.label?.match(/[0-9]×/)) return `${periodly} Tuples` as const;
